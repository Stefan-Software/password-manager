/**
 * Bootstrap already exists in HTML DOM from CDN
 */
declare var bootstrap: any

/**
 * Storage Unit
 * @param {string} 0 = Username value
 * @param {string} 1 = Server value
 * @param {string} 2 = Password value
 */
interface storageUnit extends Array<string> {
    0: string
    1: string
    2: string
}

interface globalStorage extends Array<storageUnit> {
    [index: number]: storageUnit
}

/**
 * An interface representing user input fields.
 */
interface userInputInterface extends Object {
    username: {
...
/**
 * An object representing user input fields.
 * @interface
 */
...
const userInput: userInputInterface = {
    ...
    server: {
        element: document.getElementById('Server') as HTMLInputElement,
        value: ''
    },
    password: {
...
const urlSafeRegex: RegExp = /^[a-z0-9\-._~:/?#[\]@!$&'()*+,;=%]+$/
...
document.getElementById('submit-button')!.addEventListener('click', () => {
    ...
    // Retrieve Data
    const globalStorage: globalStorage = JSON.parse(localStorage.getItem('storage') as string)
